...
database.init_db()
database.Prizes.add_prize("Beer", 50, "blahblah")
...
def api_userinfo():
    ...
    """
    result = []
    result.append({'name': userinfo().username, 'money': userinfo().points,
...
def purchase(itemnr):
    if 'username' in session and itemnr:
        database.Prizes.get_prize_from_id(itemnr)
        return True
    else:
        return False
